...
def get_attn_map(attn, pos):
	...
	for i in range(c):
		a = attn[i, :, idx[i], :]
		attns.append(a)
	attn = np.stack(attns, axis=0) # [c, n_heads, 784]
	attn = attn.reshape([c, n, 28, 28])
	return attn
...
def vis_attn_map(img, attn, hmap, size=128):
	...
	attn = np.concatenate([attn, attn.sum(axis=1, keepdims=True)], axis=1)
