...
def render_heatmap(np.ndarray[float, ndim=2, mode='c'] pts, int size, float sigma):
	cdef int n_pts = pts.shape[0]
	hmap = np.zeros([n_pts, size, size], dtype=np.float32)
	for i in range(n_pts):
		if pts[i,2] > 0:
			_get_gaus(<float*> np.PyArray_DATA(hmap[i]), size, sigma, pts[i,0], pts[i,1])
	return hmap
