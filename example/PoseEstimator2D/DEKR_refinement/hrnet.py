...
class Body(M.Model):
	...
	def forward(self, x, return_shallow=False):
		x = self.c1(x)
		s1 = x = self.c2(x)
		s2 = x = self.layer1(x)
		x = self.stage1([x,x])
...
class AdaptConvBlk(M.Model):
	def initialize(self, outchn):
		self.c = M.AdaptConv3(outchn, batch_norm=True, usebias=False)
...
class DEKR(M.Model):
	def initialize(self, num_pts):
		...
		for _ in range(num_pts):
			self.reg_blks_off.append(AdaptConvBlk(15))
			self.reg_blks_off.append(AdaptConvBlk(15))
			self.convs_off.append(M.ConvLayer(1, 2))

	def forward(self, x, return_shallow=False):
		if return_shallow:
			x, shallow = self.backbone(x, return_shallow)
		else:
			x = self.backbone(x)
		...
		if return_shallow:
			return hmap, offsets, shallow
		return hmap, offsets

class SamplingLayer(M.Model):
	def initialize(self, out_size):
		x = Parameter(torch.linspace(-1.0, 1.0, steps=out_size, dtype=torch.float32))
		y = Parameter(torch.linspace(-1.0, 1.0, steps=out_size, dtype=torch.float32))
		yy, xx = torch.meshgrid(x, y)
		self.xx = Parameter(xx)
		self.yy = Parameter(yy)
		self.out_size = out_size

	def test_forward(self, x, offset):
		return x 

	def forward(self, x, offset):
		offset = offset * 2 / self.out_size
		xx = offset[:, 0] - self.xx 
		yy = offset[:, 1] - self.yy 
		xxyy = torch.stack([xx,yy], dim=-1)  # [N, o, o, 2]
		res = F.grid_sample(x, xxyy, align_corners=True)   # [N, c, o, o]
		return res 

class DEKRRefine(M.Model):
	def initialize(self, num_pts, out_size):
		self.dekr = DEKR(num_pts)
		self.sampling = SamplingLayer(out_size)
		self.pre_conv = M.ConvLayer(3, 32, activation=M.PARAM_RELU, batch_norm=True, usebias=False)
	...
	def build_forward(self, x):
		hmap, offsets, shallow = self.dekr(x, return_shallow=True)
		# x = torch.cat([shallow[0], shallow[1], hmap.repeat(1, 5, 1, 1), offsets.repeat(1, 2, 1, 1)], dim=1)
		x = torch.cat([shallow[0], shallow[2], hmap, offsets], dim=1)
		res = []
		for i in range(self.num_pts):
			a = self.sampling.test_forward(x, offsets[:,i*2:i*2+2])
			res.append(a)
		...
		for blk in self.refine_blocks:
			x = blk(x)
		x = self.final_conv(x)
		torch.nn.init.uniform_(self.final_conv.conv.weight, -0.005, 0.005)
		return hmap, offsets, x
	...
	def forward(self, x):
		...
		res = []
		for i in range(self.num_pts):
			a = self.sampling(x, offsets[:,i*2:i*2+2])
			res.append(a)
		x = torch.cat(res, dim=1)
		x = self.pre_conv(x)
		for blk in self.refine_blocks:
