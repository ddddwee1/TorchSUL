...
class DistributedClassifier(M.Model):
	...
	def load(self, path, start_class=0):
		def load_(pthpath, idx):
			start_idx = self.weight_idx[idx] + start_class
			end_idx = self.weight_idx[idx+1] + start_class
			mtx = torch.load(pthpath)
			self.weights[idx].data[:] = mtx[start_idx : end_idx]
		path = path.replace('\\','/')
	...
	def save(self, path):
		...
		ckpt.close()
		print('Classifier saved to: %s'%path)
